...
const users = [
    { ProductId: '1',OrderDate: '2024-06-11T01:08:12.143Z' },
    // Thêm thông tin của các người dùng khác nếu cần
];

// Địa chỉ URL của API
const apiUrl = 'http://localhost:5152/api/product/order';
...
for (const user of users) {
    for (let i = 0; i < requestCount; i++) {
        user.ProductId = i;
        requests.push(() => sendRequest(user,i));
    }
}

// Hàm gửi yêu cầu
async function sendRequest(user,i) {
    try {
        const response = await axios.post(apiUrl, { ProductId: i }, { headers: { 'Content-Type': 'application/json', 'Cookie': generateCookie() } });
...
executeBatches(requestCount, 1000).catch(error => {
